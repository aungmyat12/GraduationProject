...
public class ItemInputDisplayDBAccess extends ControlDBAccess {
	
	public Item searchItemById(String itemId) throws Exception {
		// DB接続の確立
		...
		try {
			...
			pstmt = con.prepareStatement("SELECT ITEMID, ITEMNAME, SIZE, PRICE FROM ITEM WHERE ITEMID = ?");
			pstmt.setString(1, itemId); // プレースホルダーに顧客IDをセット
