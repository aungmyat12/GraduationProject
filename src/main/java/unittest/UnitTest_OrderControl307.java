...
public class UnitTest_OrderControl307 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Item item = new Item("D001", "コーラ", null, 120);
			Tax tax = new Tax(3, 0.10, null);
			OrderDetail orderDetail = new OrderDetail(0, customer, item, "2014-03-27", -1, tax, 1);
			orderDetailList.add(orderDetail);
			OrderRegisterDBAccess orDao = new OrderRegisterDBAccess();
			orDao.registerOrder(orderDetailList);
		} catch (Exception e) {
			System.out.println("項番2成功");
			e.printStackTrace();
			System.out.println(e.getMessage());
		}
		...
		try {
			...
			Item item2 = new Item();
			Tax tax2 = new Tax();
			OrderDetail orderDetail2 = new OrderDetail(0, customer2, item2, null, 0, tax2, 0);
			orderDetailList2.add(orderDetail2);
			OrderRegisterDBAccess orDao = new OrderRegisterDBAccess();
			orDao.registerOrder(orderDetailList2);
		} catch (Exception e) {
			// TODO 自動生成された catch ブロック
			System.out.println("項番3成功");
			e.printStackTrace();
			System.out.println(e.getMessage());
		}
